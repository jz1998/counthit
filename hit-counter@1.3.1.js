...
class EmojiReaction extends s {
  ...
  render() {
    ...
    <!-- container -->
    <div class="${this?.theme === 'dark' || (this?.theme === 'system' && system_theme === 'dark') ? 'container-dark' : 'container'}">
      <!-- reactions got -->
      <div style="display: flex; gap: 0.375rem; flex-wrap: wrap; justify-content: center;">
        ${this.availableReactions.map(item => x`
          <div @click=${this._react} data-name="${item.reaction_name}" class="${item.meReacted ? 'reaction-got-reacted' : 'reaction-got-not-reacted'}" style="display: flex; user-select: none; cursor: pointer; justify-content: center; align-items: center; padding: 0.25rem 1.75rem; font-size: 3.25rem; font-weight: 1000;">
